              ...
              Row(
                children: [
                  ...
                  Expanded(child: Consumer<DashBoardProvider>(
                    builder: (context, dashProvider, child) {
                      return CustomDropdown(
                        key: ValueKey(dashProvider.selectedUsers?.sId),
                        hintText:
                            dashProvider.selectedUsers?.name ?? 'Sub category',
                        items: dashProvider.subCategoriesByCategory,
                        initialValue: dashProvider.selectedUsers,
                        displayItem: (Users? users) => users?.name ?? '',
                        onChanged: (newValue) {
                          if (newValue != null) {
                            //TODO: should complete call filterVendor
                          }
                        },
                        validator: (value) {
                          if (value == null) {
                  ...
                  Expanded(
                    child: Consumer<DashBoardProvider>(
                      builder: (context, dashProvider, child) {
                        return CustomDropdown(
                            key: ValueKey(dashProvider.selectedVendor?.sId),
                            initialValue: dashProvider.selectedVendor,
                            items: dashProvider.vendorsByUsers,
                            hintText: dashProvider.selectedVendor?.name ??
                                'Select Vendor',
                            displayItem: (Vendor? vendor) => vendor?.name ?? '',
                            onChanged: (newValue) {
                              if (newValue != null) {
                                dashProvider.selectedVendor = newValue;
                                dashProvider.updateUI();
                              }
                            },
                            validator: (value) {
                              if (value == null) {
                                return 'Please vendor';
                              }
                              return null;
                            });
                      },
