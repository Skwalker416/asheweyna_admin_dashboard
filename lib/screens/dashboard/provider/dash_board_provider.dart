...
class DashBoardProvider extends ChangeNotifier {
  ...
  List<Users> subCategoriesByCategory = [];
  List<Vendor> vendorsByUsers = [];
  ...
  //TODO: should complete filterVendor
  ...
  setDataForUpdateProduct(Product? product) {
    if (product != null) {
      ...
      final newListCategory = _dataProvider.subCategories
          .where((user) => user.categoryId?.sId == product.proCategoryId?.sId)
          .toList();
      subCategoriesByCategory = newListCategory;
      selectedUsers = _dataProvider.subCategories.firstWhereOrNull(
          (element) => element.sId == product.proUsersId?.sId);

      final newListVendor = _dataProvider.vendors
          .where((vendor) => vendor.userId?.sId == product.proUsersId?.sId)
          .toList();
      vendorsByUsers = newListVendor;
      selectedVendor = _dataProvider.vendors.firstWhereOrNull(
          (element) => element.sId == product.proVendorId?.sId);
